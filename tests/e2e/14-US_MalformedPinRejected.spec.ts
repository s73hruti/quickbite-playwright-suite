...
2. Enter store ID 4821 (valid) and PIN "1ab" (invalid: non-numeric, too short).
...
test.describe('Malformed PIN E2E Test', () => {
  test('malformed PIN is rejected with a format-specific error @TC-E2E-14 @US', async ({ page }) => {
    const loginPage = new LoginPage(page);

    await test.step('attempt login with a malformed PIN', async () => {
      await loginPage.loginAs('4821', '1ab' , 'US');
    });
